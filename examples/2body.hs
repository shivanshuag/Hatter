...
mass = Mass{mass1=2500000
           ,mass2=2500000}

body1 :: GameObject
body1 = GameObject{oid="body1"
                  ,position=V2 10 300
                  ,boundingBox=Circle (V2 24 24) 24
                  ,gameGraphic=Image Sprite{file=assetdir++"planet1.png",width=48,height=48}
                  }

body2 :: GameObject
body2 = GameObject{oid="body2"
                  ,position=V2 500 300
                  ,boundingBox=Circle (V2 10 10) 20
                  ,gameGraphic=Image Sprite{file=assetdir++"planet2.png",width=40,height=40}
                  }
...
posdiff state =
  ...
  in case (object1, object2) of
    (Just o1, Just o2) -> (position o1 + V2 12 12)  - (position o2 + V2 10 10)
    (_,_) -> V2 0 0
...
body1Wire = proc state -> do
  ...
  newposition <- positionWire identityCorrectionFunction (V2 400 400) (velocityWire identityCorrectionFunction $ V2 0 50) acceleration1Wire -< state
...
body2Wire = proc state -> do
  ...
  newposition <- positionWire identityCorrectionFunction (V2 600 400) (velocityWire identityCorrectionFunction $ V2 0 $ negate 50) acceleration2Wire -< state
  let newobject = GameObject {oid=oid object, position=newposition, boundingBox=boundingBox object, gameGraphic=gameGraphic object}
  returnA -< newobject
